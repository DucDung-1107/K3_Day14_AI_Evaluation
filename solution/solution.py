...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        ap_sum = 0.0
        for k_minus_1, rel in enumerate(is_relevant):
            k = k_minus_1 + 1
            if rel:
    ...
    def score_response(
        ...
        try:
            ...
            for criterion in rubric:
                ...
                try:
                    final_scores[criterion] = float(val)
                except (ValueError, TypeError):
                    final_scores[criterion] = 0.5
        except Exception:
            final_scores = {criterion: 0.5 for criterion in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) > 1:
            ...
            for item in scores_batch:
                s = item.get("scores", {})
                if s:
                    avg_scores.append(sum(s.values()) / len(s))
            if len(avg_scores) > 1:
                # check if first score is greater than the average of the rest
                rest_avg = sum(avg_scores[1:]) / len(avg_scores[1:])
                positional_bias = avg_scores[0] > rest_avg
        ...
        avg_batch = sum(all_vals) / len(all_vals) if all_vals else 0.5
        leniency_bias = avg_batch > 0.8
        severity_bias = avg_batch < 0.3
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_f = sum(r.faithfulness for r in new_results) / n_len if n_len > 0 else 0.0
        new_avg_r = sum(r.relevance for r in new_results) / n_len if n_len > 0 else 0.0
        new_avg_c = sum(r.completeness for r in new_results) / n_len if n_len > 0 else 0.0

        base_avg_f = sum(r.faithfulness for r in baseline_results) / b_len if b_len > 0 else 0.0
        base_avg_r = sum(r.relevance for r in baseline_results) / b_len if b_len > 0 else 0.0
        base_avg_c = sum(r.completeness for r in baseline_results) / b_len if b_len > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            ...
            fix = suggestions[i] if i < len(suggestions) else "Review pipeline"
    ...
    def generate_improvement_suggestions(
        ...
        if any("hallucination" in k for k in cat_lower):
            suggestions.append("Implement hallucination checker to filter unsupported claims")
            suggestions.append("Refine prompt to instruct model to only use provided context")
        if any("incomplete" in k or "completeness" in k for k in cat_lower):
        ...
        if any("irrelevant" in k or "off_topic" in k or "topic" in k for k in cat_lower):
            suggestions.append("Improve prompt clarity and relevance constraints")
            suggestions.append("Train intent detection to better classify incoming questions")

        general = [
            "Verify retriever performance and adjust chunk overlap",
            "Review user feedback to calibrate evaluation rubrics",
            "Optimize context selection and ranking"
        ]
        for g in general:
